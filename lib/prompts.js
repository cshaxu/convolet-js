...
exports.defaultPromptBuilders = void 0;
const hasInput = (input) => Object.keys(input).length > 0;
const buildFullPrompt = (prompt, input) => [
    ...(hasInput(input)
        ? [
            "--- INPUT DATA BEGINS ---",
...
const botStream = (prompt, hasPreviousMessages, input) => buildFullPrompt([
    prompt,
    "",
...
const botEvaluation = (prompt, input) => buildFullPrompt([
    prompt,
    "",
    "--- TIPS FOR YOUR RESPONSE ---",
    `Please review the task description${hasInput(input) ? " and input data" : ""},`,
    "then generate the output data according to the output schema.",
    "",
    `IMPORANT: ONLY ADD INFORMATION THAT IS PRESENT IN THE ${hasInput(input) ? "INPUT DATA" : "TASK DESCRIPTION"}.`,
    `- If the information is missing from the ${hasInput(input) ? "input data" : "task description"} to generate certain fields`,
    "  described in the output schema, please ignore those fields in the output data.",
    `- Do not add or make up any information that is not present in the ${hasInput(input) ? "input data" : "task description"},`,
...
const botDecision = (nextNodeOptions, input) => buildFullPrompt([
    ...
    ...nextNodeOptions
        ...
        .map((option) => `- nodeKey: ${option.nodeKey}, criteria: ${option.prompt}`),
    "--- OPTIONS END ---",
    "",
].join("\n"), input);
...
exports.defaultPromptBuilders = defaultPromptBuilders;
