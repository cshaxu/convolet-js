...
function validateFlowConfig(flowConfig) {
    ...
    nodes.forEach(function (nodeConfig) {
        ...
        nextNodeOptions
            .map(function (nno) { return nno.nodeKey; })
            .forEach(function (nextNodeKey) {
            var nextNodeConfig = nodes.find(function (nc) { return nc.nodeKey === nextNodeKey; });
            if (nextNodeConfig === undefined) {
                throwError(flowKey, "NodeConfig (".concat(nodeKey, "): missing next NodeConfig (").concat(nextNodeKey, ")"));
            }
        });
        switch (nodeType) {
            case types_1.NodeType.INTERACTION:
                ...
                if (nextNodeConfig !== undefined &&
                    nextNodeConfig.nodeType !== types_1.NodeType.EXECUTION) {
